...
# -------------------------------
IDS = [0, 1, 2, 3, 4, 5]   # 6 ID validi per DICT_4X4_50 (0..49)
MARKER_MM = 75             # lato marker (mm)
DPI = 300                  # risoluzione di stampa
OUT_DIR = "aruco_4x4_50_75mm"

# Impaginazione su A4 (opzionale)
MAKE_A4 = True
A4_MARGIN_MM = 12
A4_GAP_MM = 10
...
def ensure_aruco():
    if not hasattr(cv2, "aruco"):
        raise ImportError("Modulo cv2.aruco assente. Installa: pip install opencv-contrib-python")
    if not hasattr(cv2.aruco, "DICT_4X4_50"):
        raise ImportError("La costante DICT_4X4_50 non esiste in questa build di OpenCV.")
    return True
...
def render_marker(dictionary, marker_id, side_px, border_bits=1):
    """
    Rende un marker ArUco come immagine uint8 (0-255) usando l'API disponibile:
    - OpenCV nuovi:   aruco.generateImageMarker(dict, id, side, img, borderBits)
    - OpenCV 4.5.x:   aruco.drawMarker(dict, id, side, img=None, borderBits=1)
    """
    ...
    raise RuntimeError("Né generateImageMarker né drawMarker disponibili nella tua build di OpenCV.")

def save_png(img, path):
    ok = cv2.imwrite(path, img)
    if not ok:
        raise IOError(f"cv2.imwrite ha fallito nel salvare: {path}")

# -------------------------------
# Generazione marker singoli
# -------------------------------
...
if __name__ == "__main__":
    ...
    log(f"[INFO] aruco.generateImageMarker? {hasattr(cv2.aruco,'generateImageMarker')}  |  "
    ...
    try:
        ...
        if MAKE_A4:
            ...
            log(f"[FILE] {a4}")
        log("[DONE] Fai stampa al 100% (scala 1:1) per ottenere 75×75 mm.")
    except Exception as e:
        log("[ERROR] Eccezione durante la generazione:")
        traceback.print_exc()
        sys.exit(1)
