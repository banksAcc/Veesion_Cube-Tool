...
def draw_marker_outline(img: np.ndarray, corners: np.ndarray,
    ...
    """Disegna il perimetro di un marker dato l'array (4,2) dei suoi corner."""
...
def draw_small_axes(img: np.ndarray, K: np.ndarray, dist: np.ndarray,
    ...
    """Disegna gli assi locali (X=rosso, Y=verde, Z=blu) con lunghezza axis_len."""
...
def project_points_camframe(K: np.ndarray, dist: np.ndarray,
    ...
    """Proietta punti espressi nel frame CAMERA."""
...
def draw_wirecube(img: np.ndarray, K: np.ndarray, dist: np.ndarray,
    ...
    """Disegna un cubo wireframe centrato in tvec, orientato da rvec."""
    h = cube_size * 0.5
    pts = np.array(
